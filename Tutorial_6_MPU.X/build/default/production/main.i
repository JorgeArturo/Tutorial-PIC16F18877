...
void MPU_Initialization(void){
    ...
    i2c_write1ByteRegister(0x68,0x6B,0x80);
    _delay((unsigned long)((200)*(32000000/4000.0)));
    i2c_write1ByteRegister(0x68,0x68,0x07);
    _delay((unsigned long)((200)*(32000000/4000.0)));

    i2c_write1ByteRegister(0x68,0x6B,0x01);

    i2c_write1ByteRegister(0x68,0x19,19);
    i2c_write1ByteRegister(0x68,0x1A,0x06);
    i2c_write1ByteRegister(0x68,0x1B,0x00);
    i2c_write1ByteRegister(0x68,0x1C,0x00);

    i2c_write1ByteRegister(0x68,0x6A,0x04);
    _delay((unsigned long)((20)*(32000000/4000.0)));

    i2c_write1ByteRegister(0x68,0x23,0xF8);
    i2c_write1ByteRegister(0x68,0x6A,0x40);
...
void MPU_ReadFIFO(mpu_data* adata){
    ...
    fifo_counter = i2c_read2ByteRegister(0x68,0x72);
    ...
    if(sizePacket > 0){
        ...
        for(uint8_t i=0;i<sizePacket;i++){
            ...
            i2c_readDataBlock(0x68,0x74,&fifo_data,sizeof(fifo_data));

            adata->ax += ((float)((fifo_data[0] << 8)| fifo_data[1]) / 16384);
            adata->ay += ((float)((fifo_data[2] << 8)| fifo_data[3]) / 16384);
            adata->az += ((float)((fifo_data[4] << 8)| fifo_data[5]) / 16384);

            adata->temp += (((((int16_t)fifo_data[6] << 8)| fifo_data[7])/340) + 36.53);

            adata->gx += ((((int16_t)fifo_data[8] << 8)| fifo_data[9]) / 131);
            adata->gy += ((((int16_t)fifo_data[10] << 8)| fifo_data[11]) / 131);
            adata->gz += ((((int16_t)fifo_data[12] << 8)| fifo_data[13]) / 131);
        ...
        adata->pitch = (atan2f(adata->ax,sqrtf(powf(adata->ay,2) + powf(adata->az,2))) * 57.295779) + (adata->gx * 0.1);
        adata->roll = (atan2f(adata->ay,adata->az) * 57.295779) + (adata->gy * 0.1);
...
# 202 "main.c"
    ...
    printf("0x%X\r\n",i2c_read1ByteRegister(0x68,0x75));
